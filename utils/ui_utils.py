...
# BUG FIX ✅: _append_and_update now syncs live_df to session_state BEFORE
# calling update_data_to_gsheet so that AI Insights, Summary, and all other
# pages immediately see the new row after a Credit / Debit form submission.
def _append_and_update(df, sheet, new_row, success_msg):
    new_df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    # Sync live_df first — guarantees every page sees the update on next render
    ...
    with st.form("credit_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Add Credit Details") and amount > 0:
            ...
            new_row = {
                ...
                'credit': amount, 'credit_details': source or 'NA',
    ...
    with st.form("debit_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Add Debit Details") and amount > 0:
            ...
            new_row = {
                ...
                'debit': -abs(amount), 'debit_details': note or 'NA',
                'category': category,
            }
...
def show_summary(yearly_data):
    ...
    for month in df['month'].dt.strftime('%B').unique():
        ...
        st.markdown(
            f"**Total Credited:** <span style='color:green; font-size:22px'>₹{credit:,}</span>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"**Total Debited:** <span style='color:red; font-size:22px'>₹{debit:,}</span>",
            unsafe_allow_html=True,
        )
        color = 'green' if savings >= 0 else 'red'
        st.markdown(
            f"**Savings:** <span style='color:{color}; font-size:22px'>₹{savings:,}</span>",
            unsafe_allow_html=True,
        )
        ...
        st.dataframe(
            display_df.style.apply(apply_custom_style_row, axis=1),
    ...
    st.markdown(
        f"<h1 style='text-align:center;color:{color};'>₹{final_saving:,}</h1>",
        unsafe_allow_html=True,
    )
