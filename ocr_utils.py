...
# NOTE: Tesseract path is NOT set at module level.
# It is set lazily inside _init_tesseract() so the app
# doesn't crash / show errors on pages that don't use OCR.
...
def detect_category(text, trans_type):
    """Auto-detect best-fit category from receipt text."""
...
# ============================================================
# TESSERACT PATH AUTO-DETECT (lazy — called only when OCR is used)
# ============================================================
def get_tesseract_path():
    ...
    if system == "Windows":
        ...
        # Linux — check common paths and PATH
...
def _init_tesseract():
    """
    Configure pytesseract path and return (path_or_None, ok: bool).
    Called lazily — only when OCR is actually used.
    """
...
def _normalize_ocr_text(text: str) -> str:
    """Normalize common OCR misreads in receipt text."""
    # Normalize currency symbols (₹ might be read as R, `, |, etc.)
    text = re.sub(r'[\|\`]\s*', '₹', text)
    # Tesseract often reads ₹ as z/Z/€ — normalize these before digits
    text = re.sub(r'\b[zZ]\s*(?=\d)', '₹', text)
    text = re.sub(r'€\s*(?=\d)', '₹', text)
    # FIX: Tesseract reads ₹ as '2' (visual similarity of curves)
    # "₹4,170" → OCR → "24,170"  →  fix back to "₹4,170"
    # Guard: only when '2' follows a space/tab so "₹25,000" (2 follows ₹) is safe
    text = re.sub(
...
def _parse_amount(raw: str) -> float | None:
    """
    Clean a raw matched string into a float.
    Handles Indian formatting like 25,000 or 1,25,000.
    Returns None if the value is outside a plausible transaction range (1–9,99,999).
    """
    try:
...
def parse_receipt_text(text):
    """
    Extract amount, date, merchant from OCR receipt text.

    Strategy: ONLY trust amounts that are preceded by a currency symbol (₹/Rs/INR)
    or a common Tesseract mis-read of ₹ (%, £, &, #, @, etc.).
    We never fall back to bare numbers — that causes years, IDs, and phone
    numbers to be mistaken for amounts.
    """

    text = _normalize_ocr_text(text)

    def _clean_amount(raw: str) -> float | None:
        """Strip commas/spaces, reject year-range values, return float."""
        try:
            cleaned = raw.replace(',', '').replace(' ', '').strip()
            if not cleaned:
                return None
            val = float(cleaned)
            # Explicitly reject anything that looks like a year (1900-2099)
            if 1900 <= val < 2100:
                return None
            if 1 <= val <= 9_99_999:
                return val
    ...
    def _is_id_line(line: str) -> bool:
        """True if line is a transaction ID / UTR / account number / phone."""
    ...
    # ── Tier 1: currency-symbol-anchored numbers ──────────────────────────────
    # Matches ₹25,000 / Rs.500 / INR 1200 AND common OCR mis-reads of ₹:
    #   % £ & # @ * ~ ^ ?
    # We intentionally do NOT skip account/ID lines here because UPI receipts
    # often show  "4613XXXXXXXX5928   ₹25,000"  on the same line.
    ...
    # ── Tier 2: keyword-anchored numbers (only if Tier 1 found nothing) ───────
    if not amounts_found:
        ...
        for line in lines:
            if _is_id_line(line):
                continue
            # Skip date-only lines to avoid capturing a year
    ...
    # NO Tier 3 bare-number fallback — too error-prone (years, IDs, roll numbers)

    # ── Tier 3: Indian-format comma numbers in non-ID lines ───────────────────
    # e.g. "4,170" or "25,000" — commas make these unambiguously NOT IDs/dates.
    # (?<![0-9]) ensures a stray leading digit (like '2' from ₹ misread) is not
    # absorbed — _normalize_ocr_text should fix "24,170"→"₹4,170" first anyway.
    # Only fires if Tiers 1 & 2 both found nothing.
    if not amounts_found:
...
# ============================================================
# HELPER — save entry to sheet and update session state
# ============================================================
...
def show_ocr_page(df, sheet):
    ...
    # Tesseract check happens HERE — not at import time
    ...
    with col2:
        ...
        if override_type == 'debit':
            ...
            source   = st.text_input("🏢 Source (Company / Person)",
                                     parsed['merchant'] or "", key="ocr_source")
            date_val = st.text_input("📅 Date (DD-MM-YYYY)",
                                     default_date_str, key="ocr_date_c")
            amount   = st.number_input("💰 Amount (₹)",
                                       min_value=0.0, value=default_amount,
                                       step=1.0, format="%.2f", key="ocr_amount_c")
            category = st.selectbox("📂 Income Type",
                                    credit_categories, index=cat_index, key="ocr_cat_c")

            if st.button("➕ Add Income", type="primary",
                         use_container_width=True, key="ocr_add_credit"):
                if amount <= 0:
                    st.error("❌ Amount must be greater than 0.")
                else:
                    _save_ocr_entry(
